...
| 工作区 | tracked/untracked/ignored 状态、整文件暂存、分组批量暂存/取消暂存；暂存切换以及已暂存或未暂存的已跟踪文件重置期间保留现有列表与历史，以标题进度表达写入且保持复选框原选中颜色，只在 Git 完成后原子替换工作区状态，仍有改动时保持 `Uncommitted changes`，工作区变干净时选择最新提交；从 `Uncommitted changes` 或“文件状态”手动刷新时保留入口及有效文件选择并清除失效选择；暂存和未暂存列表均可经确认删除所选工作区文件且不直接修改索引；普通已跟踪文本文件的未暂存 Diff 区块支持单独暂存或确认后放弃，已暂存区块支持单独取消暂存，刷新后恢复入口和仍有效的文件选择；同时改变已有文件模式（含 executable bit）的 Diff 不提供区块操作；左侧“工作区”提供“文件状态”和“历史”入口，前者完整展示工作区文件、Diff 和提交入口，后者的 `Uncommitted changes` 在提交图内选中并显示下方改动面板 | 行级暂存、过滤、丢弃预览 | sparse checkout 增强 |
| Diff | Unified Diff、文本/二进制识别、大文件降级 | 左右对比、语法高亮、空白选项 | 可扩展渲染器 |
| 提交 | Commit、Amend、作者校验、hook 错误展示 | 模板、签名状态 | 提交草稿 |
| 历史 | 分页列表、基础 DAG、提交详情、双击或右键检出提交、将选定提交合并到当前分支、从选定提交创建分支、标签、复制 SHA-1、推送、变基、重置、回滚、创建补丁和遴选；历史提交中的文本新增、修改和删除 Diff 支持确认后按区块反向应用到当前工作区，不改写原提交，同时改变已有文件模式（含 executable bit）时不提供区块回滚；历史提交文件列表的“查看选中的修改日志…”会以路径过滤已加载本地分支历史、跟踪单文件重命名，并按各提交的实际路径在独立只读视图显示真实摘要和 Unified Diff；关闭或切换会取消过期读取，非 UTF-8 路径入口保持禁用并标注“（待实现）”；内置审查已支持所选历史文件；文件级“重置到提交…”会确认后将该提交的路径版本写入 index/worktree，不移动 HEAD，并拒绝过期选择或进行中 Git 操作；“复制路径到剪贴板”会在复核历史选择后复制仓库相对路径；“打开当前版本”、Finder 和 Quick Look 由原生层复核当前工作树文件后执行；“打开已选定版本”二进制安全读取有大小上限的 Git blob，通过私有临时文件交给默认应用并随工作区清理，删除记录保持禁用；外部 Diff 已使用 Apple FileMerge 比较第一父提交的前后原始 blob；自定义操作仍直接标注“（待实现）”且只显示无副作用提示；变基/回滚/遴选要求干净工作区，提交级 hard reset 会列出将丢弃的已跟踪路径并要求二次确认；非提交对象标签保留在导航但不进入提交详情 | 搜索、Blame、Reflog | 高级查询 |
| Branch / Tag | 创建/切换/重命名本地分支、从指定本地或已获取远端分支创建分支、引用右键菜单、安全删除已合并分支；左侧本地分支叶节点默认使用无彩色图标的紧凑文本行，当前选中分支保留蓝色圆点，目录层级、选择和操作行为保持不变；读取本地 Tag、从历史提交创建轻量/附注 Tag、精确推送或确认删除本地/指定远端 Tag | Tag 签名与远端 Tag 状态管理 | Worktree |
...
- 应用操作完成后立即刷新；文件监控已按每仓库窗口实现 debounce/coalesce：普通工作区变化只重读
  status、操作状态和当前 Diff，Git 元数据变化才重载引用与历史，写操作期间的事件合并为一次后续刷新。
- 窗口重新获得焦点时已完整校准状态、引用和历史；平台监听不可用时保留工具栏手动刷新。
...
- 每个工作区 Engine 独立监听 worktree、Git directory 与 common directory；400ms 防抖并设 5 秒最大等待，自动读取完成后至少冷却 1 秒。普通文件与当前 worktree 的 index 变化只刷新 status、操作状态和当前工作区 Diff；HEAD、refs、packed-refs、配置及合并/变基状态才刷新完整仓库快照，objects、日志、锁文件和其他不影响可见状态的 Git 内部事件不会触发重载。刷新读取开始前监听已就绪，期间事件按请求代际合并为后续补读；监听事件在应用 Git 写操作期间延后合并，窗口恢复前台时完整校准状态、引用和历史；监听释放、事件升级、交错关闭和外部修改刷新由测试覆盖。
...
- macOS 单进程多窗口宿主：首页使用初始 Flutter Engine，每个工作区在当前进程创建并持有独立
  ...
  自动重建对应工作区；退出前的合并组会保存明确的成员与标签顺序，部分拆出的窗口在重启后继续独立。恢复时只在目标成员完成 Git 验证后收拢该组，启动后新建或快照中独立的窗口不会被吸收；旧版“全部合并”布尔偏好自动迁移为全路径成员列表。恢复批次等待上限为 30 秒；超时只结束批次等待并保留对应窗口，迟到的目标组成员验证完成后仍可重新加入，迟到的独立窗口只完成登记，不阻塞持久化，也不擅自关闭用户窗口。合并后不创建系统胶囊标签条，只在标题栏正下方显示 29pt 高、按标签数量等分整行的矩形标签；新打开仓库在 Git 验证成功后会自动加入现有合并组。每个按钮铺满标签条高度，不保留顶部空隙，标签使用居中文字、1pt 分隔线、长名称省略与完整悬停提示。标签支持原生拖动排序，松手后更新窗口协调器顺序和下一次启动的恢复路径顺序。每个标签提供独立关闭按钮，并通过对应窗口的 `prepareToClose` 与 Engine 生命周期完成安全清理；窗口协调器关闭合并窗口的系统显隐动画，并在切换时原位同步仓库标题和选中态，不重建标签层级。手动关闭单个窗口会从恢复列表移除，不可读或不再能由 Git 验证的路径会自动丢弃。关闭窗口释放对应 Engine。当前仍只支持一个合并组，多个并存窗口组列入后续范围。
  用户主动拆出工作区时会同步从待恢复组意图中移除该路径，迟到的 Git 验证回调不会把它重新并入。
...
- 工作区状态、未跟踪目录文件展开、整文件 stage/unstage、Command 多选、批量切换、暂存与未暂存列表中经确认删除工作区文件（索引保持不变，确认后重读状态并拒绝过期来源；批量操作逐项反馈结果，非 UTF-8 路径标注“移除（待实现）”）、对已暂存或未暂存已跟踪修改/删除文件的“重置…”（明确确认后将 index 与 worktree 一起恢复到 HEAD；写入前按所选暂存区或工作区来源复核最新 Git 状态，成功或状态校验拒绝后，只要仍有其他改动就保持 `Uncommitted changes` 选中），以及停止追踪普通已追踪改动或已暂存新增文件（从 index 移除而不删除本地文件；已有提交文件确认后显示为已暂存删除与同路径未跟踪文件，已暂存新增文件恢复为未跟踪）。文件列表始终按 Git 原始暂存/未暂存状态展示，不维护应用私有的停止追踪分组。另有 Unified Diff、基础历史 DAG 和提交详情；提交图采用
...
- [x] “动作”菜单的“添加到索引”和“从索引中取消暂存”使用独立 action ID，按当前 key
  ...
  入口依据最新仓库视图二次校验。
- [x] “动作”菜单接入 Finder、系统 Terminal、Quick Look 和系统默认应用打开；路径快照按 Engine
  隔离并限制在工作树内，AppKit 执行前复核存在性，多选路径不会部分执行。
- [x] 历史文件“外部差异比对”按第一父提交读取二进制安全的前后 blob，新增/删除使用空内容作为
  缺失侧，并通过私有临时快照交给 Apple FileMerge；不执行仓库配置的 external diff/textconv。
- [x] “窗口”菜单接入填充、居中、靠左和靠右，并覆盖首页与工作区两类 key window；几何始终
  限制在目标屏幕 `visibleFrame`，窄屏贴靠继续遵守 900px 最小宽度。
- [x] “窗口”菜单接入合并标签的循环切换、移出和标签栏显隐；移出保留独立 Engine，会话内剩余
  两个以上窗口继续成组，只剩一个时解除组；明确保存组成员和顺序，重启后恢复“标签组 + 独立窗口”。
- [x] “窗口”菜单动态列出在线显示器，标记并禁用当前屏幕；移动时保持相对中心位置并将窗口完整
  约束到目标 `visibleFrame`，目标较小时安全收缩。
- [~] “窗口”菜单已接入键盘可访问的合并组标签总览，选择后切换至目标独立
  Engine，组变更时关闭过期总览；全屏平铺和多个持久化窗口组待实现。
- [x] 已覆盖多个 workspace 快速循环切换、后台仓库菜单快照隔离、窗口关闭/
  Engine 失效以及过期菜单更新拒绝；测试确认关闭窗口不再成为原生菜单目标。

#### M2：选择相关工作流与恢复路径

- [x] 工作区文件右键菜单的“创建补丁…”按当前选择导出相对 `HEAD` 的完整二进制 Diff；执行前
  重读 Git 状态并复核暂存来源，重命名包含旧路径，16 MB 上限和排他发布避免截断或覆盖目标。
- [x] “提交所有…”预览并提交全部已跟踪改动及已有暂存内容但排除纯未跟踪文件；“提交选中项…”只提交当前可见选择中各路径的完整工作区版本并保留其他已暂存文件，提交前重读来源，失败后显示刷新后的部分状态；“动作 → 提交…”复用同一选择级流程；“检出…”支持本地分支、已获取远端分支和已加载提交；“添加远程仓库…”只修改本地配置且不自动连接。
- [x] 文件历史已从原生“动作”菜单接入当前单个已跟踪工作区文件或历史提交文件，并复用只读
  history 用例；ignore 已支持精确路径/扩展名预览及仓库共享/本机排除两种目标；复制和移动已支持
  普通工作区文件的目标预览、状态复核、取消和不覆盖发布，移动会区分同卷原子重命名与跨卷
  复制后安全删除；内置审查支持工作区多选文件和历史文件的只读 Unified Diff，并取消过期读取。
  工作区文件右键菜单也已接入 Terminal、Quick Look、单文件历史、多文件内置审查和 ignore
  规则预览，并可打开既有补丁检查/应用面板；执行前按最新仓库视图复核明确选择，系统文件动作
  再由 AppKit 验证当前 Engine、路径边界和完整存在性。工作区“创建补丁…”仍等待选择级补丁语义。
- [x] merge/rebase/cherry-pick/revert 的 Continue/Abort 已接入按当前 key workspace
  操作状态动态命名和校验的原生菜单；冲突子菜单支持对当前单个冲突文件使用 stage 2、
  stage 3 或标记为已解决，并按操作解释版本来源，执行前刷新 Git 状态并拒绝过期选择。
- [ ] 实现多个并存窗口组和受支持 macOS 版本的系统全屏平铺，并补充显示器热插拔恢复测试。
...
- [x] 仓库级 Reset 可从已加载历史选择目标，历史选择可直接“重置到提交”；提交级重置默认 mixed，
  ...
  历史提交文件级“重置到提交…”则只覆盖单一路径的 index/worktree，不移动 HEAD；执行前重读状态、
  复核提交和路径选择并拒绝冲突操作，支持 detached HEAD 及提交中已删除路径的恢复语义。
- [x] 将已交付的文件列表“移除”及批量处理能力接入 macOS 原生“动作”菜单；继续区分
...
- `.github/workflows/quality.yml` 在 pull request 和 `main` 推送时执行 `lib/`、`test/`、
  `integration_test/` 与 `tool/` 的格式检查、`flutter analyze`、工具静态分析、完整 `flutter test`
  与 macOS Debug 构建；非 PR 的 `main` 任务还执行
  Runner XCTest。
- 每周定时任务和手动触发执行 `integration_test/macos_core_workflow_test.dart`；测试只能使用
  本地 bare remote 与隔离临时目录，不能依赖真实凭据或公网 Git 服务。
- 仓库托管平台应将 `Flutter quality` 配置为 `main` 的必需合并检查。该分支保护属于托管平台
  外部状态，未配置前 workflow 只能报告结果，不能阻止直接合并。

### 性能基准基础

- `tool/generate_benchmark_fixture.dart` 固定 seed 离线生成 Small（1,000 commits/files、20 refs）、
  Medium（10,000 commits/files、200 refs）和 Stress（100,000 commits/files、1,000 refs、10,000
  未提交改动）仓库。生成器只接受不存在或空的目标目录，并以空模板及隔离 Git 配置运行，
  不使用用户 hooks、凭据、全局配置或网络。
- `tool/benchmark_repository.dart` 对已有仓库只读采集 status、refs 和固定 revision snapshot 的
  500 条历史，输出可保存的每轮、median 与 P95 JSON；它可与已保存基线比较 P95，默认相对回退
  超过 15% 时以非零状态退出。Small/Medium 应在性能参考机上记录基线，Stress 只在定时或手动
  环境运行。
...
- 已完成一轮低风险 UI 卡顿优化：历史 Graph 后备拓扑按提交页身份缓存，引用导航的平面分区
  ...
  地址失败时会清理 loading 状态并允许重新选择远端。尚未完成
  引用导航的全量 Widget 懒构建和搜索输入防抖，需在独立变更中配套性能测量与回归测试。

### 仓库信任边界

- “仓库详情…”提供按 Git common-dir 与 worktree 身份持久化的“未确认（默认限制）/信任/受限”
  选择；默认不从目录、用户名或远端推断信任，用户可随时改为受限。
- 信任仅控制应用选择开放的外部扩展。它不改写 Git global/system 配置、`safe.directory`、hooks、
  credential helper、SSH 配置或远端；未确认和受限状态拒绝应用自有的自定义操作、外部 Diff 和
  textconv。当前这些外部扩展仍未交付。
